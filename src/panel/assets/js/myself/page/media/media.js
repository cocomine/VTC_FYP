...
define([ 'jquery', 'toastr', 'bootstrap' ], function (jq, toastr, bootstrap){
    ...
    $('#media-list').on('click', '.media-list-focus', function (){
        ...
        if (select_mod){
            ...
            detail.find('p:nth-child(4) > code').text(media.mime);
            detail.find('p:nth-child(5) > code').text(location.origin + '/panel/api/media/' + id);
    ...
    /* 修改圖片 */
    $('#file-name').blur(changeName).keyup(function (e){
        if (e.key === "Enter") changeName(e);
    });
