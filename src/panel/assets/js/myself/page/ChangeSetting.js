...
define([ 'jquery', 'toastr', 'zxcvbn', 'forge', 'bootstrap', 'FileSaver', 'media-select', 'media-select.upload', 'full.jquery.crs.min', 'intl-tel-input', 'moment', 'myself/datepicker' ],
    (jq, toastr, zxcvbn, forge, bootstrap, FileSaver, media_select, media_upload, crs, intlTelInput, moment) => {
        "use strict";
        ...
        /* 處理生日日期輸入 */
        const jq_birth = $('#birth-date-picker');
        if (jq_birth.children('input').attr('value').length <= 0){
            jq_birth.children('input').val(moment().subtract(15, 'y').format('YYYY-MM-DD'));
        }
        jq_birth.children('input').attr('max', moment().subtract(15, 'y').format('YYYY-MM-DD'));
        jq_birth[0].datepicker.draw();
        ...
        $("#organize-phone, #phone").on('input focus', function (){
            ...
            /* 修改 phone number input html */
            .after(`<div class="invalid-feedback">請輸入正確電話號碼</div>`).parent().children('.iti__flag-container').css({ 'height': jq_phone.outerHeight() + 'px' });
